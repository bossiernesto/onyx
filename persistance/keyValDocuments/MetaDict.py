from config.AppContext import appContext,ONYX_ROOT,getAppContext
...
def getContainerSection():
    return appC.get(CONTAINER_SECTION,)
...
class BaseDocumentSession(object):
    ...
    def remove(self):
        appContext().getInstance().remove(self.get_section(self.appC),self)
